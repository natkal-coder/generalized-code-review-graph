...
PLATFORMS: dict[str, dict[str, Any]] = {
    ...
    "gemini-cli": {
        "name": "Gemini CLI",
        "config_path": lambda root: Path.home() / ".gemini" / "settings.json",
        "key": "mcpServers",
        "detect": lambda: (Path.home() / ".gemini").exists() or shutil.which("gemini") is not None,
        "format": "object",
        "needs_type": False,
    },
    "antigravity": {
...
_GEMINI_MD_SECTION_MARKER = "<!-- code-review-graph MCP tools -->"
...
def inject_platform_instructions(repo_root: Path) -> list[str]:
    ...
    manual file scanning. Uses platform-specific content when available.
    ...
    for label, filename in _PLATFORM_INSTRUCTION_FILES.items():
        ...
        # Use Gemini-specific instructions for GEMINI.md, Claude section for others
        if filename == "GEMINI.md":
            if _inject_instructions(path, _GEMINI_MD_SECTION_MARKER, _GEMINI_MD_SECTION):
